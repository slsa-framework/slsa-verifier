...
func GetRekorEntriesWithCert(rClient *client.Rekor, artifactHash string, provenance []byte) (*dsselib.Envelope, *x509.Certificate, error) {
...
func VerifyProvenanceSignature(ctx context.Context, rClient *client.Rekor, provenance []byte, artifactHash string) (*dsselib.Envelope, *x509.Certificate, error) {
	...
	if env, cert, err := GetRekorEntriesWithCert(rClient, artifactHash, provenance); err == nil {
		return env, cert, nil
	}
